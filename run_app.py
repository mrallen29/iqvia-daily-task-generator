...
class App(BaseWindow):
    def __init__(self):
        ...
        ttk.Label(self.header_frame, text='SOD/EOD Report Generator', font=('Segoe UI', 14, 'bold')).pack(side='left')
...
# Badges
    ...
    def load_config(self):
        ...
        self.config['UPDATE_MANIFEST_URL'] = str(self.config.get('UPDATE_MANIFEST_URL', '') or '').strip()
        try:
    ...
    def auto_check_for_updates_on_startup(self):
        try:
            if bool(self.config.get('AUTO_CHECK_UPDATES', False)) and str(self.config.get('UPDATE_MANIFEST_URL', '') or '').strip():
    ...
    def check_for_updates(self, show_no_update: bool = True, startup: bool = False):
        ...
        if not manifest_url:
            if show_no_update:
                messagebox.showinfo('Updates', 'No Update Manifest URL is saved yet. Go to Settings > Updates and save one first.')
            return False
    ...
    def save_update_settings(self):
        try:
            manifest_url = self.update_manifest_entry.get().strip()
        except Exception:
            manifest_url = str(self.config.get('UPDATE_MANIFEST_URL', '') or '').strip()
...
# Navigation handled via header buttons
    ...
    def create_help_widgets(self):
        ...
        ttk.Label(tab_updates, text='Update Manifest URL:').pack(padx=10, pady=(5, 5), anchor='w')
        ...
        ttk.Button(updates_btn_row, text='Save Update Settings', command=self.save_update_settings).pack(side='left')
